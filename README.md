...
**Interactive tutorial:** [`notebooks/PINN_Nozzle_Solver.ipynb`](notebooks/PINN_Nozzle_Solver.ipynb) — <a href="https://colab.research.google.com/github/harsh147-github/PINNs---Bernoulli/blob/main/notebooks/PINN_Nozzle_Solver.ipynb" target="_parent"><img src="https://colab.research.google.com/assets/colab-badge.svg" alt="Open In Colab"/></a> a from-scratch teaching notebook for readers with zero prior ML/PINN exposure: starts with a trivial 1-line ODE to build the concept from nothing (autodiff, residuals, training with no data), then walks through this repo's actual nozzle code section by section — geometry, the hard-BC trick, physics residuals, a full worked derivation of the loss function, live training, validation against an independent RK45 solver, and an honestly-measured inference-speed benchmark — ending with a build-your-own-PINN checklist.
